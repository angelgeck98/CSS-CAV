...
class LidarSpoofEarlyAttacker(Attacker):
    def __init__(self):
        ...
        # Load a spoof vehicle mesh
        self.mesh = o3d.io.read_triangle_mesh("car_0200.ply")
        self.mesh.compute_vertex_normals()
        self.raycasting = o3d.t.geometry.RaycastingScene()
        self.mesh_id = self.raycasting.add_triangles(o3d.t.geometry.TriangleMesh.from_legacy(self.mesh))

    def spoof(self, attacker_vehicle, lidar_points):
        """
        Inject spoofed points in front of the attacker vehicle.

        Parameters:
        - attacker_vehicle: the CARLA actor to spoof from
        - lidar_points: original LiDAR points (Nx4 array from CARLA)

        Returns:
        - spoofed_points: original + spoofed points
        """
        ...
        # Move the spoof mesh to the new spoofing position
        spoofed_mesh = copy.deepcopy(self.mesh)
        spoofed_mesh.translate(spoof_loc)

        # Re-register the translated mesh to the raycasting scene
        self.raycasting.clear()
        self.mesh_id = self.raycasting.add_triangles(o3d.t.geometry.TriangleMesh.from_legacy(spoofed_mesh))
        ...
        # Perform raycasting
        ans = self.raycasting.cast_rays(o3d.core.Tensor(rays))
        ...
        # Merge spoofed points with original LiDAR points
